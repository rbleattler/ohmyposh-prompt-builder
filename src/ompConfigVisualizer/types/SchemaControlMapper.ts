...
export class SchemaControlMapper {
  ...
  /**
   * Get controls configuration for a specific segment type
   */
  public getControlsForSegmentType(segmentType: string): Record<string, any> | null {
    if (!this.schema) return null;

    // Look for segment type in schema definitions
    const segmentDefinitionPath = `#/definitions/segment/properties/type/enum`;
    const segmentDefinition = this.getPropertyByPath(segmentDefinitionPath);

    // Check if this segment type is valid
    if (Array.isArray(segmentDefinition) && !segmentDefinition.includes(segmentType)) {
      return null;
    }

    // Get properties schema for this segment type
    const propertiesSchemaPath = `#/definitions/segments/${segmentType}/properties`;
    const propertiesSchema = this.getPropertyByPath(propertiesSchemaPath);

    if (!propertiesSchema || typeof propertiesSchema !== 'object') {
      return null;
    }

    // Map each property to a control configuration
    const controls: Record<string, any> = {};

    for (const [propName, propSchema] of Object.entries(propertiesSchema)) {
      if (typeof propSchema !== 'object') continue;

      const result = this.mapPropertyToControl(propName, propSchema as any);
      if (result && result.controlConfig) {
        controls[propName] = result.controlConfig;
      }
    }
  ...
  /**
   * Get a property from the schema by its path
   */
  private getPropertyByPath(path: string): any {
    try {
      if (path.startsWith('#/')) {
      ...
      console.error(`Error accessing schema path ${path}:`, error);
      return null;
    }
  }

  /**
   * Map a JSON schema property to a UI control configuration
   */
  mapPropertyToControl(propName: string, propSchema: any, required: boolean = false): {
    ...
    if (propSchema.$ref) {
      const resolvedProperty = this.resolveReference(propSchema.$ref);
      if (!resolvedProperty) return null;
      return this.mapPropertyToControl(propName, resolvedProperty);
    }

    // Default control configuration


    // Determine control type based on property type


    return {
      propertyDef: this.mapSchemaToPropertyDefinition(propName, propSchema, required),
      controlConfig: this.getControlConfig(this.mapSchemaToPropertyDefinition(propName, propSchema, required))
    };
  }

  /**
   * Format a property name for display (e.g., convert camelCase to Title Case)
   */
  private formatPropertyName(name: string): string {
    return name
      ...
      .replace(/([a-z])([A-Z])/g, '$1 $2')
      // Insert a space after each underscore and remove the underscore
      .replace(/_/g, ' ')
